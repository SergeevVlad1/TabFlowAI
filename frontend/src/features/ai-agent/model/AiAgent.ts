...
class ProductivityAiAgent {
  ...
  private analyzeTabs() {
    ...
    if (tabs.length > 8 && groups.length === 0) {
      addInsight({
        ...
        action: () => console.log('Action: Group Tabs triggered')
      });
    }

    const socialMediaTabs = tabs.filter(t => t.url.includes('facebook.com') || t.url.includes('twitter.com') || t.url.includes('youtube.com'));
    if (socialMediaTabs.length > 2) {
      addInsight({
        type: 'warning',
        message: 'High distraction potential detected. You have multiple social media tabs open.',
  ...
  private analyzeWorkSessions() {
    const { currentSession, getDailyDuration } = useTrackingStore.getState();
    const { addInsight } = useAiStore.getState();

    if (currentSession && currentSession.type === 'focus') {
      const durationMin = (Date.now() - currentSession.startTime) / 1000 / 60;
      if (durationMin > 50) {
        addInsight({
          type: 'kudos',
          message: 'Great focus session! You have been working for over 50 minutes. Time for a short break?',
          actionLabel: 'Take Break',
          action: () => useTrackingStore.getState().startSession('break')
        });
      }
    }
    ...
    if (focusTime < 30 && new Date().getHours() > 14) {
      addInsight({
        type: 'optimization',
        message: 'Your focus time is low today. Try the Pomodoro technique for the next hour.',
      });
    }
  }
}
